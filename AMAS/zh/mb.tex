...
\documentclass[twoside,a4paper,UTF8]{ctexart}
\usepackage{amsmath,amssymb}            % 用于数学环境和符号
\usepackage{graphicx}           % 用于图形导入（替代epsfig，可处理多种格式图片）
\usepackage{fancyhdr}
\usepackage{fontenc}
%--------------------------Page Format--------------------------
%\headsep 0.5 true cm
...
\fancypagestyle{fancy}{
  \fancyhf{}
  \fancyhead[CE]{\protect{\zihao{-5}\songti \hfill  应 \quad  用 \quad  数
  \quad  学 \quad  学 \quad  报} \hfill 34\,{\zihao{-5}\songti  卷}}
  \fancyhead[CO]{\protect 5\,{\zihao{-5}\songti  期} \hfill \TT \hfill}
}
\pagestyle{fancy} % 然后按照需求设置页眉和页脚格式
